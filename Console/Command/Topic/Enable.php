...
class Enable extends Command
{
    ...
    {
        $this->setName('amazon-sns:topic:enable')
            ->setDescription('Enable an SNS topic command')
            ->setDefinition([
                new InputArgument(
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $topic = reset($topic);
        if ($topic) {
            $topic->setIsActive(1)->save();
            $result = 'Topic has been enabled successfully!';
        } else {
